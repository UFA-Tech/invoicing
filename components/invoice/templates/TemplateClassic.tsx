...
const styles = StyleSheet.create({
  ...
  brandName: { fontSize: 22, fontFamily: "Helvetica-Bold", color: "#0f172a" },
  businessInfo: { fontSize: 9, color: "#64748b", lineHeight: 1.6 },
  invoiceMeta: {
    flexDirection: "row",
  ...
  invoiceTitle: { fontSize: 28, fontFamily: "Helvetica-Bold", color: "#0f172a", marginBottom: 4 },
  invoiceNumber: { fontSize: 12, color: "#64748b" },
  metaGrid: { flexDirection: "column", alignItems: "flex-end", gap: 4 },
  metaRow: { flexDirection: "row", gap: 12 },
  metaLabel: { fontSize: 9, color: "#94a3b8", width: 72, textAlign: "right" },
  metaValue: { fontSize: 9, fontFamily: "Helvetica-Bold", color: "#0f172a", width: 80, textAlign: "right" },
  ...
  tableHeader: { flexDirection: "row", backgroundColor: "#f1f5f9", borderRadius: 4, padding: "8 10", marginBottom: 4 },
  ...
  tableCellText: { fontSize: 9, color: "#374151" },
  totals: { alignItems: "flex-end", marginBottom: 32 },
  totalRow: { flexDirection: "row", width: 220, padding: "4 0" },
  totalLabel: { flex: 1, fontSize: 9, color: "#64748b" },
  totalValue: { fontSize: 9, color: "#0f172a", fontFamily: "Helvetica-Bold" },
  grandTotalRow: { flexDirection: "row", width: 220, padding: "10 12", backgroundColor: "#0f172a", borderRadius: 6, marginTop: 8 },
  grandTotalLabel: { flex: 1, fontSize: 10, color: "#94a3b8", fontFamily: "Helvetica-Bold" },
  grandTotalValue: { fontSize: 11, color: "#ffffff", fontFamily: "Helvetica-Bold" },
  notes: { marginBottom: 16, padding: 16, backgroundColor: "#f8fafc", borderRadius: 6, borderLeft: "3px solid #e2e8f0" },
  notesTitle: { fontSize: 8, fontFamily: "Helvetica-Bold", color: "#94a3b8", textTransform: "uppercase", marginBottom: 6 },
  notesText: { fontSize: 9, color: "#64748b", lineHeight: 1.6 },
  footer: { borderTop: "1px solid #e2e8f0", paddingTop: 16, flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
...
export function TemplateClassic({ invoice, business }: Props) {
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <View>
            ...
            {business.businessAddress && <Text style={styles.businessInfo}>{business.businessAddress}</Text>}
            {business.businessPhone && <Text style={styles.businessInfo}>{business.businessPhone}</Text>}
            {business.businessEmail && <Text style={styles.businessInfo}>{business.businessEmail}</Text>}
            {business.taxNumber && <Text style={styles.businessInfo}>NPWP: {business.taxNumber}</Text>}
          </View>
        </View>
        ...
        <View style={styles.table}>
          ...
          {invoice.items.map((item, i) => (
            <View key={i} style={styles.tableRow}>
              <Text style={[styles.colDesc, styles.tableCellText]}>{item.description}</Text>
              ...
              <Text style={[styles.colPrice, styles.tableCellText]}>{fmt(item.unitPrice, invoice.currency)}</Text>
              <Text style={[styles.colAmount, styles.tableCellText]}>{fmt(item.amount, invoice.currency)}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={styles.footer}>
          <Text style={styles.footerText}>{business.businessName ?? "Bisnis Anda"} — Terima kasih atas kepercayaan Anda.</Text>
